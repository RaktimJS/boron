...
def dictMaker(queryString, jsonSchema, jsonFilePath):
        """
...
def read(queryString, jsonSchema, jsonFilePath):
        def print_nested(data, prefix="", level=4):
                """Helper function to print nested dictionary structures"""
                ...
                for key, value in data.items():
                        ...
                        if isinstance(value, dict):
                                ...
                                print_nested(value, prefix="", level=level + 4)
                        else:
                                print(f"{indent_str}{prefix}{key}: {value}")

        Dict = dictMaker(queryString, jsonSchema, jsonFilePath)
